...
$('#axlp_cliente').on('change', function(){
...
var centro_costo = $('#centro_costo_id').val();
var id_transferencia = $('#transferencia_id').val();
...
var url_ajax_tabla_detalle = route('transferencia.editarListadoProductos', id_transferencia);
...
function cargarLosProductosModal(){
    ...
    var url_ajax = route('transferencia.obtenerListadoProductosEditar', [lista_id, ubicacion_id, nuevo_metodo_pago, centro_costo]);
...
$('#form_actualizar_transferencia').on('submit', function(e) {
    event.preventDefault();
    $('.btn_guardar').prop('disabled', true);
    if ($('#form_actualizar_transferencia')[0].checkValidity() === false) {
        ...
        // agregar data
        var $thisForm = $('#form_actualizar_transferencia');
        var formData = new FormData(this);

        //ruta
        var url = route('transferencia.actualizarTransferencia');
        ...
        $.ajax({
            ...
            dataType:'json',
            beforeSend:function(){
                swal("Actualizando transferencia, espera porfavor...", {
                    button: false,
                    timer: 3000
                });
            }
            ...
            if (!respuesta.error) {
                ...
                swal('Transferencia Actualizada Correctamente', {
                    icon: "success",
                    button: false,
                    timer: 3000
                });
                window.location.href = route('transferencia.todasTransferencias');
    ...
    $('#form_actualizar_transferencia').addClass('was-validated');
