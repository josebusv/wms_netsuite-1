...
function cargarDatosTablaProductos() {
    ...
    var centro_costos = $('#idcentrocosto').val();
    ...
    var url = route('tomar_pedido.obtenerListadoProductosEditar', [lista_id, ubicacion_id, metodo_pago, centro_costos]);
    console.log(url);
...
function mortardetalletable() {
    ...
    setTimeout(function () {
        ...
        console.log(datos_select_productos.length)
        if (validar > 0) {
            for (index = 0; index < clickInputCantidad.length; index++) {
                console.log(clickInputCantidad[index] + "/" + datos_select_productos[index].cantidad);
                ...
                $('#txt_sin_iva_' + clickInputCantidad[index] + '').val(datos_select_productos[index].sin_iva);
                $('#txt_con_iva_' + clickInputCantidad[index] + '').val(datos_select_productos[index].con_iva);
    ...
    $('#input_calcular_cant_' + params + '').on('keyup', function (evt) {
        ...
        if (evt.key === 'Tab') {
            ...
            if (cantidad > 0) {
                ...
                if (!clickInputCantidad.includes(params)) {
                    console.log(clickInputCantidad.includes(params))
                    clickInputCantidad.push(params)
...
function obtenerDatosTableProductos() {
    ...
    setTimeout(function () {
        ...
        console.log(clickInputCantidad.length)
        for (index = 0; index < clickInputCantidad.length; index++) {
...
function functionCalcularTotales(datos) {
    ...
    datos.forEach(element => {
        cantidad_items++;
        subtotal = subtotal + parseFloat(element.sin_iva.replace(',', ''));
        total = total + parseFloat(element.con_iva.replace(',', ''));
    });
